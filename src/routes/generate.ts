...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // We need to re-generate the prompt/negPrompt locally just to return them in metadata
    // (Or we could have returned them from the worker, but this is cheaper/faster than changing return types)
    // Actually, let's just use defaults for the response metadata since the images are what matter
